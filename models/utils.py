...
def fps_subsample_3pc(pcd1, pcd2, pcd3, n_points=2048):
    """
    Args
        pcd1: (b, n, 3)
        pcd2: (b, n, 3)
    returns
        new_pcd1: (b, n_points, 3)
        new_pcd2: (b, n_points, 3)
    """
    idx = furthest_point_sample(pcd1, n_points)
    new_pcd1 = gather_operation(pcd1.permute(0, 2, 1).contiguous(), idx)
    new_pcd1 = new_pcd1.permute(0, 2, 1).contiguous()
    new_pcd2 = gather_operation(pcd2.permute(0, 2, 1).contiguous(), idx)
    new_pcd2 = new_pcd2.permute(0, 2, 1).contiguous()
    new_pcd3 = gather_operation(pcd3.permute(0, 2, 1).contiguous(), idx)
    new_pcd3 = new_pcd3.permute(0, 2, 1).contiguous()
    return new_pcd1, new_pcd2, new_pcd3


def fps_subsample_with_dist(pcd, dist, n_points=512):
    """
    Args
        pcd: (b, N, 3)
        dist: (b, N, 1)
    ...
    returns
        new_pcd: (b, n_points, 3)
        new_dist: (b, n_points, 3)
    """
    idx = furthest_point_sample(pcd, n_points)
    new_pcd = gather_operation(pcd.permute(0, 2, 1).contiguous(), idx)
    new_pcd = new_pcd.permute(0, 2, 1).contiguous()
    new_dist = gather_operation(dist.permute(0, 2, 1).contiguous(), idx)
    new_dist = new_dist.permute(0, 2, 1).contiguous()
    return new_pcd, new_dist
...
def fps_subsample_with_max_dist(pcd, max_dist, n_points=512):
    """
    Args
        pcd: (b, N, 3)
        max_dist: (b, N, 1)
    ...
    new_pcd = gather_operation(pcd.permute(0, 2, 1).contiguous(), idx)
    new_pcd = new_pcd.permute(0, 2, 1).contiguous()
    new_max_dist = gather_operation(max_dist.permute(0, 2, 1).contiguous(), idx)
    new_max_dist = new_max_dist.permute(0, 2, 1).contiguous()
    return new_pcd, new_max_dist
...
def group_knn(query_xyz, xyz, k):
    """
    Args:
        query_xyz: Tensor, (B, 3, N)
        xyz: Tensor, (B, 3, m)
        k: int
    Returns:
        grouped_xyz: Tensor, (B, 3, N, k)
    """
    idx = query_knn(k, xyz.permute(0, 2, 1).contiguous(), query_xyz.permute(0, 2, 1).contiguous())
